...
/**
 * Action to explan the config form from collapsed state
 * @param {object} context The context for this test
 * @ return {Error} returns an error if the expand link is not clicked, else true
 */
async function expandConfig( { driver } ) {
  return await clickButtonOrLink(driver, By.css(EXPAND_CONFIG), 'Expand config not found');
}
...
async function expandConfigAndClickNewBlankDraft( { driver } ) {
  ...
  return await clickButtonOrLink(
    driver,
    By.xpath("//*[contains(text(), 'New Blank Draft')]"),
    'New draft button not found'
  );
}
