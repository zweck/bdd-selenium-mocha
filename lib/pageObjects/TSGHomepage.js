...
/**
 * Action to click on Home nav link and verify the elements'
 * @param {object} context The context for this test
 */
async function clickHomelink( { driver } ) {
  await clickButton(driver, By.linkText("HOME"), 'Not able to find Home link');
}
...
async function graphIsPresent( { driver } ) {
  ...
  if( graphText != "Planned vs Actual" ) {
    return error({ driver, message: `Coudln't find any text Planned vs Actual for Graph` });
  } else {
    return true;
  }
}
