...
const _tsgNavContainer = (driver) => {
  return elementIsPresent(driver, By.css('.main-nav div'));
};

const _dtmNavContainer = (driver) => {
  return elementIsPresent(driver, By.css('.main-nav div div'));
};
...
async function navigateToDtmHomePage( { driver } ) {
  ...
  const link = await elementIsPresent(dtmNav, By.xpath("//a[text()='Home']"), 'Could not find DTM Home link');
  return await link.click();
}
...
async function navigateToDtmDraftPage( { driver } ) {
  const dtmNav = await _dtmNavContainer(driver);
  const link = await elementIsPresent(dtmNav, By.xpath("//a[text()='Drafts']"), 'Could not find DTM Drafts link');
  await link.click();
  return await _draftPageHasLoaded(driver);
}
