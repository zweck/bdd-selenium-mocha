...
async function pageHasLoadedWithPlan(driver){
  await driver.wait(until.elementLocated(By.css('.plan-header header .plan-header__view-planning-level')), DEFAULT_WAIT_TIME, `Couldn\'t find Planning Level Button in Header after ${DEFAULT_WAIT_TIME}ms`);
  await driver.wait(elementIsNotPresent(driver, By.css('.loading-indicator')), DEFAULT_WAIT_TIME, `Loading indicator didn't disappear after ${DEFAULT_WAIT_TIME}ms`);
