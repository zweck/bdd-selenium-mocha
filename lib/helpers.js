...
async function hasLoaded(driver, { rootElement, name }){
  ...
  console.log(chalk.green(`     ${name} LOADED     `))
  return await driver.wait(until.elementLocated(By.css(rootElement)), DEFAULT_WAIT_TIME, `Couldn\'t find ${name} after ${DEFAULT_WAIT_TIME}ms`);
}
